...
| `container tracing.xlsx` 缓存副本 | RLGU2503769 的 2026-01-29 租赁记录和 2026-02-04 `GATE-OUT EMPTY` | 全批运输完成或 off-hire |
| `Gate In status from Owner 2026.05.21.xlsx` 缓存副本 | owner 记录 RLGU2503769 于 2026-04-24 gate-in；并列出截至 2026-05-21 的其他逐柜 gate-in 状态 | gate-in 自动等同 off-hire |
...
有效的逐箱状态推进包括 RLGU2503769 的 `LEASED → GATE_OUT_EMPTY → GATE_IN_RECORDED`、RLGU2503814 的 `LEASED → GATE_OUT_FULL`，以及 RLGU2503711 的 `GATE_IN_EMPTY → OFF_HIRE_RECORDED`。Founder 确认的全批 off-hire 作为独立的批次事件保存，不伪装成逐箱堆场记录。
